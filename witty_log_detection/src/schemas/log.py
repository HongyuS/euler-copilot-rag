...
class LogModel(BaseModel):
    ...
    template_vector: list[float] | None = Field(default=None, description="日志的嵌入向量表示")
    save_path: str | None = Field(default=None, description="日志存储路径")
    is_anomalous: bool = Field(default=False, description="日志是否异常")
    anomaly_reason: str = Field(default="", description="日志异常原因，如果日志不异常，则返回空字符串")
    anomaly_score: float = Field(default=0.0, description="日志异常分数，如果日志不异常，则返回0.0")
    # 并查集参数
