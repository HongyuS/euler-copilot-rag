...
class TokenTool:
    stop_words_path = Config().get_config().static_source.stop_words_path
    ...
    def filter_stopwords(content: str) -> str:
        ...
        try:
            ...
            logger.exception("[TokenTool] %s", err)
        return content
    ...
    def get_leave_tokens_from_content_len(content: str) -> int:
        ...
        for i in range(len(grades) - 1):
            if grades[i].content_len <= tokens < grades[i + 1].content_len:
                index = i
                break
        leave_tokens = grades[index].tokens + (
            grades[index + 1].tokens - grades[index].tokens
        ) * (tokens - grades[index].content_len) / (
            grades[index + 1].content_len - grades[index].content_len
        )
    ...
    def get_leave_setences_from_content_len(content: str) -> int:
        ...
        for i in range(len(grades) - 1):
            if grades[i].content_len <= len(sentences) < grades[i + 1].content_len:
                index = i
                break
        leave_sentences = grades[index].tokens + (
            grades[index + 1].tokens - grades[index].tokens
        ) * (len(sentences) - grades[index].content_len) / (
            grades[index + 1].content_len - grades[index].content_len
        )
