...
class EmbeddingConfigModel(BaseModel):
    """向量化服务配置模型"""
    type: str = Field(default="openai", description="服务类型")
    api_key: str = Field(default="", description="API密钥")
    endpoint: str = Field(default="", description="服务端点URL")
...
class PublicConfigModel(BaseModel):
    ...
    embedding: EmbeddingConfigModel = Field(default_factory=EmbeddingConfigModel, description="向量化服务配置")
...
class BaseConfig():
    ...
    def __init__(self) -> None:
        ...
        if framework_config_file and os.path.exists(framework_config_file):
            ...
            # 读取框架的 embedding 配置
            if hasattr(framework_config, 'embedding') and framework_config.embedding:
                self._config.public_config.embedding.type = framework_config.embedding.type
                self._config.public_config.embedding.endpoint = framework_config.embedding.endpoint
                self._config.public_config.embedding.api_key = framework_config.embedding.api_key
                self._config.public_config.embedding.model_name = framework_config.embedding.model
