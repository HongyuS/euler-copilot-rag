...
async def weighted_keyword_and_vector_search(
    ...
    :param query: 查询文本
    :param top_k: 返回数量
    :param keyword_weight: 关键词搜索权重（0-1之间），向量权重自动为 1 - keyword_weight
    :param doc_ids: 可选的文档ID列表，用于过滤
    :param banned_chunk_ids: 可选的被禁用的chunk ID列表，用于过滤
    :return: 合并后的 chunk 列表
    """
    # 计算向量权重
    weight_keyword = keyword_weight
    weight_vector = 1.0 - keyword_weight
    try:
        ...
        # 关键词搜索
        try:
            keyword_chunks = ChunkManager.search_by_keyword(conn, query, 2 * top_k, doc_ids, banned_chunk_ids)
        ...
        if Embedding.is_configured():
            try:
                ...
                if query_vector:
                    vector_chunks = ChunkManager.search_by_vector(conn, query_vector, 2 * top_k, doc_ids, banned_chunk_ids)
            except Exception as e:
