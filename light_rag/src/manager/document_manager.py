"""
文档操作模块 - 使用 SQLAlchemy ORM，含异步导入任务执行逻辑
"""
import asyncio
import json
import logging
...
from sqlite.kb_sqlite import Chunk, Document, KnowledgeBase
...
async def run_document_import_task(task_id: str) -> None:
    """
    执行文档导入任务（子进程入口，需独立初始化 task.db）
    """
    ...
    db_path = os.path.join(_src_dir, "database", "kb.db")
    db = Database(db_path)
    ...
    for i, fp in enumerate(file_paths):
        ...
        try:
            ok, msg, data = await import_document(file_session, kb_id, fp, chunk_size)
            if ok:
            ...
            file_session.close()
        pct = 100.0 * (i + 1) / len(file_paths)
        await TaskManager.update_task_by_id(task_id, {"completion_precent": pct})

    result_summary = json.dumps({"success_files": success_files, "failed_files": failed_files})
    final_status = TaskStatusEnum.SUCCESSFUL_PENDING_REMOVE.value if success_files else TaskStatusEnum.FAILED_PENDING_REMOVE.value
    await TaskManager.update_task_by_id(task_id, {"status": final_status, "result_summary": result_summary})
